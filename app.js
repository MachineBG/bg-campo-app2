...
async function trpcMutation(procedure, input) {
  ...
  // tRPC v10 expects input wrapped in {0: {json: input}} for batch, or just POST to procedure
  ...
  // Handle array response (tRPC batch)
  if(Array.isArray(json)) json = json[0];
  
  if (!res.ok || json?.error) {
    const err = json?.error;
    const msg = err?.message || err?.json?.message || 
